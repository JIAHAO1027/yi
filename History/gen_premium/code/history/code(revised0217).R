...
deltan<-read.csv("700d_relative_premuin_withbundle1.csv")
result3<-result2%>%left_join(deltan,by=c("ItemID.LIsting_id."="ItemID.LIsting_id.","bundle_index"="bundle_index"))
result3$diff=as.character(result3$diff)
result3$diff=as.numeric(result3$diff)
result3$diff[is.na(result3$diff)]=0

setwd("../output")
write.csv(result3,file = "numofacc_and_premium_700D_yi_0218.csv")
