...
deltan<-read.csv("delta_n_stepwise.csv",stringsAsFactors = FALSE)
...
setwd("../output")
write.csv(result3,file = "numofacc_and_premium_700D_adjacent_0220.csv")
...
hist(result2$prem_adjacent,main="histogram of prem_adjacent",xlab="prem_adjacent")
dev.off()
...
boxplot(result3$bundle_index~result3$diff,main="Delta n vs Bundle Index",xlab="delta_n",ylab="bundle_index")
dev.off()
...
model2=lm(result3$prem_adjacent~result3$bundle_index+ factor(result3$item_id) -1 )
summary(model2)
