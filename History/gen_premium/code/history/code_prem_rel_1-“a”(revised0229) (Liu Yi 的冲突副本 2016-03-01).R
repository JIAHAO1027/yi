...
#using dict_index rather than dict_yi_index
...
result<-CameraWPrice%>%
  ...
  summarise(total=sum(Accessory_title!=""),
            ...
            num_of_yuanzhuang=sum(as.vector(sapply('Ô­×°£…',grepl,Accessory_title))),
...
result2=result%>%filter(premium_minus_a>-500&premium_minus_a<500)
png(file="boxplot- bundle_index~premium_minus_a(zoom).png",bg="transparent")
boxplot(result2$premium_minus_a~result2$bundle_index,xlab="bundle_index",ylab="premium_minus_a",main="premium_minus_a vs. bundle_index")
dev.off()
