...
library(lfe)
####################################################fixme
#setwd("C:/Users/xin_chen/Dropbox/urap_programming/yi/gen_yi_low")
setwd("F:/Dropbox/urap_programming/yi/gen_premium_yi_low")
...
Price<-read.csv(filename_dict,encoding = "UTF-8",stringsAsFactors = FALSE)%>%select(index_final,Avg..Price.Esitimate)
...
Price_low=read.csv(filename_dict_yi_low,stringsAsFactors = FALSE)%>%select(Index_temporary,filtered_low)
Price=Price%>%left_join(Price_low,by=c("index_final"="Index_temporary"))
Price$Avg..Price.Esitimate=ifelse(!is.na(Price$filtered_low),Price$filtered_low,Price$Avg..Price.Esitimate)
...
###fix me
pnp(file="share-bundle_index.png")
